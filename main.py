...
def print_banner():
    ...
    console.print(Panel(
        Text.assemble(
            ...
            ("  ─────────────────────────────────────────────────\n", "dim"),
            ("  Type a question  │  'demo' for showcase  │  'queries' to browse  │  'dashboard' for charts  │  'quit' to exit", "dim italic"),
        ),
        box=box.DOUBLE_EDGE,
        border_style="cyan",
        padding=(0, 2),
...
DEMO_QUERIES = [
    ...
    # ⚡ Combo — all agents fire together
    "Full audit: performance review across all business lines, identify fatigued creatives, and give me a budget reallocation plan",
    "Which business lines are below ROAS target, which creatives are fatigued, and where should I shift budget this week?",
]

# ── All sample queries per agent type (shown via 'queries' command) ──
ALL_SAMPLE_QUERIES = {
    "📊  DATA ONLY": [
        "Show total spend and ROAS by platform this month",
        "List all campaigns below their ROAS target",
        "Show daily spend and impressions for the last 7 days",
        "What is the total revenue generated by WB Games this month?",
        "Show ad sets with the highest CPM on Google",
    ],
    "🔍  ANALYSIS AGENT": [
        "Give me a full performance review across all business lines this month",
        "Why is Max Streaming underperforming and what should we do?",
        "Compare Meta vs TikTok performance — which deserves more budget?",
        "How has ROAS trended over the past 30 days across all platforms?",
        "Which business lines are hitting their ROAS targets and which are not?",
        "What is happening with Theatrical campaigns — performance review?",
    ],
    "🎨  CREATIVE ANALYST AGENT": [
        "Which creative format has the best ROAS across all platforms?",
        "Which creatives need urgent refresh? Give me a priority list",
        "How are Video 15s vs Static Image creatives performing?",
        "Which Reel and Story formats are working best on Meta?",
        "Compare TikTok video performance vs YouTube Pre-roll",
        "What creative formats should we test that we are not running yet?",
    ],
    "💰  OPTIMIZER AGENT": [
        "I have SGD 50K extra budget — where should I allocate it for maximum ROAS?",
        "Which platforms are we over-investing in relative to ROAS?",
        "Rebalance my entire budget to maximise blended ROAS",
        "Should I shift budget from Google to TikTok? Show me the numbers",
        "Give me a full budget optimisation plan for next month",
        "If I need to cut total budget by 20%, what is the smartest way to do it?",
    ],
    "⚡  COMBO (All Agents)": [
        "Full audit: performance review, creative health, and budget optimisation",
        "Which business lines are below target, which creatives are fatigued, and where should I shift budget?",
        "Give me the complete picture: what is working, what is not, and what should change?",
        "Performance review + identify top fatigued creatives + budget reallocation recommendation",
    ],
}
...
def print_queries():
    """Show all available sample queries grouped by agent type."""
    console.print()
    console.rule("[bold bright_cyan]  SAMPLE QUERIES — BY AGENT TYPE[/bold bright_cyan]", style="cyan")
    console.print()
    for agent_label, queries in ALL_SAMPLE_QUERIES.items():
        console.print(f"  [bold bright_white]{agent_label}[/bold bright_white]")
        for q in queries:
            console.print(f"    [dim]•[/dim] [white]{q}[/white]")
        console.print()
    console.print("  [dim italic]Copy any query above and paste it at the prompt.[/dim italic]")
    console.print()


def run_demo(orch: Orchestrator):
    console.print()
    console.rule(f"[bold cyan]DEMO MODE — {len(DEMO_QUERIES)} Showcase Queries[/bold cyan]")
    console.print(f"  [dim]Covering: Analysis · Creative Analyst · Optimizer · Combo (all agents)[/dim]\n")
